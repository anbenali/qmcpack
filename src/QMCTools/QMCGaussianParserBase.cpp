...
void QMCGaussianParserBase::dump(const std::string& psi_tag, const std::string& ion_tag)
{
  ...
  if (!Structure)
  {
    ...
    if (PBC)
    {
      app_log()<<"ABORT::THIS IS NOT SUPPOSED TO HAPPEN. PBC are ON but you are not in an HDF5 path. Contact developers"<<std::endl;
      exit(0);
        ...
        if (multideterminant)
        {
          ...
          else
          {
            if (UseHDF5)
            {
              ...
              if (usingCSF)
              {
                 app_log()<<"Warning: CSF in HDF5 not implemented. Will attempt to revert multideterminant to xml. It is recommended to verify input for accuracy or avoid using -hdf5"<<std::endl;
...
xmlNodePtr QMCGaussianParserBase::createMultiDeterminantSetFromH5()
{
  ...
  {
     app_log()<<"WARNING!! THE HDF5 Contains CI coefficients for "<<nbexcitedstates-1<<". By default, the ground state coefficients will be loaded ( ext_level=1). If you want to evaluate an excited for which the coefficients are stored in the HDF5 file, modify the value of ext_level between 1 and "<<nbexcitedstates<<std::endl; 
