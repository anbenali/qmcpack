...
xmlNodePtr QMCGaussianParserBase::createXMLforSPOSet(const std::string& name, int size, int spin)
{
  std::ostringstream size_str, spin_str;
  size_str << size;
  spin_str << spin;
  xmlNodePtr spoPtr = xmlNewNode(NULL, (const xmlChar*)"sposet");
  xmlNewProp(spoPtr, (const xmlChar*)"basisset", (const xmlChar*)"LCAOBSet");
  xmlNewProp(spoPtr, (const xmlChar*)"name", (const xmlChar*)name.c_str());
  xmlNewProp(spoPtr, (const xmlChar*)"size", (const xmlChar*)size_str.str().c_str());
  if (DoCusp)
  {
    std::string cusp_file("../" + name + ".cuspInfo.xml");
    xmlNewProp(spoPtr, (const xmlChar*)"cuspInfo", (const xmlChar*)cusp_file.c_str());
  }
  // add occupation
  xmlNodePtr occ_data = xmlNewNode(NULL, (const xmlChar*)"occupation");
  xmlNewProp(occ_data, (const xmlChar*)"mode", (const xmlChar*)"ground");
  xmlAddChild(spoPtr, occ_data);

  // add coefficients
  xmlNodePtr coeff_data = xmlNewNode(NULL, (const xmlChar*)"coefficient");
  xmlNewProp(coeff_data, (const xmlChar*)"spindataset", (const xmlChar*)spin_str.str().c_str());
  xmlAddChild(spoPtr, coeff_data);
...
void QMCGaussianParserBase::dumpNew(const std::string& psi_tag, const std::string& ion_tag)
{
  ...
  ///sposet_collection
  xmlNodePtr sposet_coll = xmlNewNode(NULL, (const xmlChar*)"sposet_collection");

  xmlNewProp(sposet_coll, (const xmlChar*)"type", (const xmlChar*)"MolecularOrbital");
  xmlNewProp(sposet_coll, (const xmlChar*)"name", (const xmlChar*)"LCAOBSet");
  xmlNewProp(sposet_coll, (const xmlChar*)"source", (const xmlChar*)ion_tag.c_str());
  if (PBC)
  {
    ...
    xmlNewProp(sposet_coll, (const xmlChar*)"twist", (const xmlChar*)(ss.str()).c_str());
    sss << Image[0] << "  " << Image[1] << "  " << Image[2];
    xmlNewProp(sposet_coll, (const xmlChar*)"PBCimages", (const xmlChar*)(sss.str()).c_str());
  }
  if (DoCusp == true)
    xmlNewProp(sposet_coll, (const xmlChar*)"cuspCorrection", (const xmlChar*)"yes");
  xmlNewProp(sposet_coll, (const xmlChar*)"href", (const xmlChar*)h5file.c_str());
  ...
  xmlNewProp(gridPtr, (const xmlChar*)"npts", (const xmlChar*)"1001");
  xmlAddChild(bset, gridPtr);
  xmlAddChild(sposet_coll, bset);
  ///Sposet collection
  int sposet_size;
  if (multideterminant)
    sposet_size = ci_nstates;
  else
    sposet_size = numMO;

  if (!SpinRestricted)
  {
    xmlAddChild(sposet_coll, createXMLforSPOSet("spo-up", sposet_size, 0));
    xmlAddChild(sposet_coll, createXMLforSPOSet("spo-dn", sposet_size, 1));
  }
  else
    xmlAddChild(sposet_coll, createXMLforSPOSet("spo-RHF", sposet_size, 0));
  xmlAddChild(wfPtr, sposet_coll);
  ///DeterminantSet
